...
def test_assistant_general_stream_router_is_mounted_by_loader():
    from routers import assistant_general_routes

    routes = {
        (method, route.path)
        for route in assistant_general_routes.router.routes
        for method in getattr(route, "methods", [])
    }

    assert ("POST", "/assistant/general/stream") in routes
