...
def test_composer_submit_reads_form_and_dom_fallback():
    ...
    assert "const finalText = (formText || input || domText).trim()" in text
...
def test_composer_textarea_is_controlled_with_message_name():
    ...
    assert "value={input}" in composer
    assert "onInput={(event) => onInputChange(event.currentTarget.value)}" in composer
