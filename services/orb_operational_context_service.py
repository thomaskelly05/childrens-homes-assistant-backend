...
def _snapshot_rows(conn: Any, current_user: dict[str, Any], *, scope: str, home_id: int | None, young_person_id: int | None, staff_id: int | None, limit: int) -> list[dict[str, Any]]:
...
def _answer_for(*, message: str, scope: str, intent: str, sources: list[dict[str, Any]], context: dict[str, Any], errors: list[str]) -> tuple[str, str, str]:
    if not sources:
    ...
    cited = sources[:5]
    evidence_lines = [f"- {item['citation_ref']} {item['title']}: {item['summary']}" for item in cited]
    open_actions = [
        item
        for item in context.get("actions") or []
        if _text(item.get("status")).lower() not in {"completed", "done", "closed", "cancelled", "canceled"}
    ]
    review_documents = [
        item
        for item in context.get("documents") or []
        if any(term in _text(item.get("status")).lower() for term in ("review", "draft", "expired", "processing", "action"))
    ]
    evidence_gaps = [
        item
        for item in context.get("evidence") or []
        if _text(item.get("quality")).lower() in {"draft", "partial", "review_required"}
    ]

    lead = {
        "today_what_changed": "Today’s operational picture is based on the most recent chronology, actions, documents and evidence I can cite.",
        "child_chronology": "The child’s chronology should be read as a care journey, not a judgement about the child.",
        "safeguarding_risk": "Risk and safeguarding records indicate areas for calm manager review.",
        "documents_templates": "Document status should be checked against sign-off, review and evidence links.",
        "inspection_sccif": "Inspection readiness should be treated as evidence-led preparation, not a predicted outcome.",
        "reg44_reg45": "Reg 44 and Reg 45 preparation should focus on source evidence, manager review and open actions.",
        "workforce": "Workforce signals should be reviewed through supervision, training, wellbeing and recording quality evidence.",
        "actions_tasks": "Open actions should be triaged by due date, safeguarding relevance and manager oversight.",
    }.get(intent, "I found live operational records in your permitted scope.")

    summary_parts = [
        f"{len(sources)} live source(s) cited for {scope}.",
        f"{len(open_actions)} open action(s) visible.",
        f"{len(review_documents)} document(s) may need review.",
        f"{len(evidence_gaps)} evidence gap(s) or draft-quality evidence item(s) visible.",
    ]
    answer = "\n".join(
        [
            lead,
            ...
            *evidence_lines,
            ...
            f"- Documents needing review/sign-off attention: {len(review_documents)}.",
            f"- Evidence gaps or draft-quality sources: {len(evidence_gaps)}.",
            "",
            "Review required: ORB is summarising live records and does not make final safeguarding, inspection or registered manager decisions.",
        ]
    )
    confidence = "high" if len(sources) >= 5 and not errors else "medium"
