"""ORB Voice realtime session governance and governed egress (NR-1 Phase 2C)."""
...
logger = logging.getLogger("indicare.orb_voice_realtime_governance")

ORB_VOICE_PRODUCT_AREA = "ORB Voice / ORB Residential"
ORB_VOICE_REALTIME_PRODUCT_ACCESS = "voice_workflows"
...
def build_orb_voice_realtime_governance_context(
    *,
    feature: str,
    purpose: str,
    route: str,
    ...
    model: str | None = None,
    product_access: str | None = ORB_VOICE_REALTIME_PRODUCT_ACCESS,
) -> AiRealtimeGovernanceContext:
    """Build safe ORB Voice realtime governance metadata (no raw instructions)."""
    ...
    return AiRealtimeGovernanceContext(
        ...
        metadata={
            "product_area": ORB_VOICE_PRODUCT_AREA,
...
def _governed_response_to_legacy_dict(
    response: AiRealtimeSessionResponse,
    *,
    conversational: bool,
) -> dict[str, Any]:
...
async def issue_orb_voice_conversational_realtime_session(
    *,
    instructions: str,
    voice: str | None,
    current_user: dict[str, Any] | None,
    ...
    """Issue a governed conversational ORB Voice realtime session."""
    ...
    governance = build_orb_voice_realtime_governance_context(
        ...
        purpose=AiRealtimeSessionPurpose.ORB_VOICE_CONVERSATIONAL.value,
        route=route,
    ...
    result = _governed_response_to_legacy_dict(response, conversational=True)
    if getattr(egress, "blocked_reason", None):
        logger.info(
            "orb_voice_realtime_session_blocked route=%s reason=%s user_id=%s instructions_len=%s",
            route,
        ...
        logger.warning(
            "orb_voice_realtime_session_failed route=%s session_id=%s user_id=%s error=%s",
            route,
            orb_session_id,
            _scope_id(current_user, "user_id") or _scope_id(current_user, "id"),
            response.error_code,
...
async def issue_orb_voice_transcription_realtime_session(
    *,
    instructions: str,
    current_user: dict[str, Any] | None,
    orb_session_id: str | None,
    ...
    """Issue a governed transcription-only ORB Voice realtime session."""
    if not orb_realtime_provider_service.provider_available():
        return {
            "provider": "openai_realtime",
            "configured": True,
    ...
    governance = build_orb_voice_realtime_governance_context(
        ...
        purpose=AiRealtimeSessionPurpose.ORB_VOICE_TRANSCRIPTION.value,
        route=route,
        instructions=instructions,
        current_user=current_user,
        orb_session_id=orb_session_id,
    ...
    response, egress = await ai_governed_egress.issue_realtime_session(request, governance=governance)
    result = _governed_response_to_legacy_dict(response, conversational=False)
    if getattr(egress, "blocked_reason", None):
        logger.info(
            "orb_voice_transcription_realtime_session_blocked route=%s reason=%s user_id=%s instructions_len=%s",
            route,
            egress.blocked_reason,
            _scope_id(current_user, "user_id") or _scope_id(current_user, "id"),
            len(instructions or ""),
