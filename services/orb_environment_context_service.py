...
ENVIRONMENT_MODES = {
    ...
    "management_review": {"tone": "oversight", "response_length": "concise", "motion": "restrained", "retrieval_priority": "evidence_gaps"},
    "manager_review": {"tone": "oversight", "response_length": "concise", "motion": "restrained", "retrieval_priority": "evidence_gaps"},
    "supervision": {"tone": "reflective_oversight", "response_length": "balanced", "motion": "low", "retrieval_priority": "supervision_continuity"},
...
class OrbEnvironmentContextService:
    def settings_for(self, mode: str = "general") -> dict[str, str | bool]:
        ...
        settings["glow_intensity"] = "dim" if resolved in {"night_shift", "quiet_hours", "child_present", "emotional_overload"} else "evidence_edge" if resolved in {"safeguarding", "inspection", "inspection_prep"} else "soft"
        settings["information_density"] = "minimal" if resolved in {"child_present", "emotional_overload", "mobile_quick_support"} else "chronology_first" if resolved in {"inspection", "inspection_prep", "safeguarding"} else "calm"
        settings["background_ambience"] = "darker_quiet_gradient" if resolved in {"night_shift", "quiet_hours"} else "privacy_softened" if resolved == "child_present" else "ambient_neural_flow"
        settings["sound_design"] = "muted" if resolved in {"child_present", "quiet_hours", "night_shift", "emotional_overload"} else "soft_confirmation_only"
        settings["retrieval_posture"] = settings["retrieval_priority"]
        return settings
