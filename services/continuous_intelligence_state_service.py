...
class ContinuousIntelligenceStateService:
    ...
    def _child_state(
        ...
        record_quality: list[dict[str, Any]],
        ...
        voice_missing = "child_voice_missing" in pattern_types
        missing_pressure = "missing_episode_increase" in pattern_types
        relationship_pressure = "repeated_family_contact_escalation" in pattern_types
        evidence_links = len(evidence_graph.get("links") or []) if isinstance(evidence_graph, dict) else 0
        return {
            ...
            "placement_stability": "fragile" if pressure >= 45 or missing_pressure else "watch" if pressure >= 20 or relationship_pressure else "stable",
            "child_voice_visibility": "limited" if voice_missing else "visible_or_not_flagged",
            ...
            "relationship_temperature": "pressure" if relationship_pressure else "not_flagged",
    ...
    def _home_state(
        ...
        safeguarding_score = self._weighted_score(patterns, include={
            "missing_episode_increase",
            "incident_increase",
            "restraint_increase",
            "safeguarding_concern_repeated",
    ...
    def _emotional_climate(
        ...
        return {
            ...
            "review_prompts": [
                "Review whether records show warmth, curiosity, repair and child voice.",
                "Check whether staff need reflective support before practice drifts into task-only care.",
            ] if score >= 20 else ["Continue recording protective factors, repair and positive progress."],
        }
    ...
    def _orb_context(
        ...
        *,
        ...
        spine: IntelligenceSpineResponse,
    ) -> dict[str, Any]:
        return {
            "context_type": "continuous_intelligence_state",
            "headline": spine.summary.headline,
            "what_orb_should_notice_first": [
                home_state.get("management_oversight"),
                child_state.get("placement_stability"),
                workforce_state.get("state"),
                evidence_state.get("evidence_strength"),
            ],
            "suggested_questions": [
                "What needs calm manager review today?",
                "Which evidence explains this pattern?",
                "What should be checked before any decision is made?",
                "Where could practice drift be developing?",
            ],
            "must_include": [
                "evidence basis",
                "human review required",
                "no automated safeguarding decision",
            ],
        }
