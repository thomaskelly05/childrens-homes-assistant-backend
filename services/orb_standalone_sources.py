...
def build_standalone_sources(
    ...
    *,
    ...
    classification = orb_expert_answer_engine_service.classify_scenario(message, mode=mode)
    family_id = classification.get("primary_family") or orb_expert_scenario_bank_service.detect_expert_context(
        message
    ).get("family_id")
    if family_id:
        registry_citations = orb_citation_decision_service.select_sources(
            family_id=family_id,
            role=classification.get("possible_roles", [None])[0] if classification.get("possible_roles") else None,
