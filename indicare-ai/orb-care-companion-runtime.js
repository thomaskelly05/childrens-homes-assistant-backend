...
if (!root) {
  throw new Error('ORB assistant root is missing')
}

if (window.__ORBCareCompanionRuntime) {
  throw new Error('Duplicate ORB Care Companion runtime detected')
}
...
function modeButtons() {
  return modes.map(mode => `
    <button class="orb-mode-pill" type="button" data-mode="${escapeHtml(mode)}" aria-pressed="${state.mode === mode}">
      ${escapeHtml(mode)}
    </button>
  `).join('')
}
...
function render() {
  root.innerHTML = `
    <section class="orb-companion-shell" data-state="${state.active ? 'active' : 'idle'}">
      ...
      <main class="orb-companion-stage">
        <div>
          <button class="orb-companion-orb ${state.active ? 'is-active' : ''} ${state.speaking ? 'is-speaking' : ''} ${state.listening ? 'is-listening' : ''}"
            type="button"
            aria-pressed="${state.active}"
            aria-label="${state.active ? 'Stop ORB voice conversation' : 'Start ORB voice conversation'}"
  ...
  root.querySelectorAll('.orb-mode-pill').forEach(button => {
    button.addEventListener('click', () => {
      state.mode = button.dataset.mode || 'Ask ORB'
      state.detail = modeDetail(state.mode)
      render()
    })
  })
}
...
function modeDetail(mode) {
  if (mode === 'Safeguarding') return 'Start with immediate safety, escalation, professional curiosity and clear recording.'
  if (mode === 'Reflect') return 'Slow the situation down and turn practice moments into learning.'
...
async function handleTypedMessage(event) {
  ...
  try {
    const response = await fetch('/orb/conversation', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      credentials: 'include',
      body: JSON.stringify({
        message: `[${state.mode}] ${message}`,
        scope: 'home',
...
async function startVoice() {
  ...
  try {
    ...
    console.error('[ORB voice] start failed', error)
    state.runtime?.stop?.()
    state.active = false
    state.connected = false
    state.speaking = false
