...
function render(){const root=document.getElementById('indicareAiRoot');if(root)root.innerHTML=layout();bind();}
function bind(){document.querySelectorAll('[data-mode]').forEach(el=>el.onclick=()=>{state.mode=el.dataset.mode;save();render();});const send=document.querySelector('[data-action="send"]');if(send)send.onclick=submit;document.querySelectorAll('[data-action="voice"]').forEach(v=>v.onclick=toggleVoice);}
async function submit(){const input=document.getElementById('prompt');if(!input||!input.value.trim())return;const text=input.value.trim();input.value='';state.messages.push({role:'user',content:text});render();try{const res=await fetch('/assistant/general-safe',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({message:text})});const data=await res.json();state.messages.push({role:'assistant',content:data.answer||data.response||'Ready.'});}catch(e){state.messages.push({role:'assistant',content:'Connection issue.'});}save();render();}
function toggleVoice(){state.voice=!state.voice;if(window.IndiCareIntelligenceLive){state.voice?window.IndiCareIntelligenceLive.start():window.IndiCareIntelligenceLive.stop();}save();render();}
