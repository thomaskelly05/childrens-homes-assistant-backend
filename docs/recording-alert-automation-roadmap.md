...
- **ISN safeguarding network** — digest, bell items, Care Hub card, brief section, recording cross-links (metadata only)
...
3. ~~**Connect notifications** — urgent/safeguarding badge events to existing notification bell (metadata only)~~ **Done** via `os_notification_adapter_service` (recording + ISN)
