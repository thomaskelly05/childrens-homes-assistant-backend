            ...
            <div className="mt-3 flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-end gap-3">
                ...
                {(process.env.NODE_ENV !== 'production' ||
                  process.env.NEXT_PUBLIC_ENABLE_ORB_DIAGNOSTICS === 'true') && (
                  <Link
                    href={`/assistant/orb/diagnostics${query.young_person_id ? `?young_person_id=${encodeURIComponent(query.young_person_id)}` : ''}`}
                    className="rounded-2xl border border-white/20 bg-white/10 px-4 py-3 text-xs font-black uppercase tracking-[0.14em] text-blue-50 hover:bg-white/15"
                  >
