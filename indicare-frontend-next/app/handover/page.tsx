...
type SearchParams = Promise<{ child_id?: string; home_id?: string; draft_id?: string }>
...
export default async function HandoverPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const homeId = params.home_id ? Number(params.home_id) : undefined
  ...
  return (
    <div className="mx-auto max-w-[1400px] px-4 py-8 sm:px-6">
      <HandoverWorkspace
        ...
        homeId={Number.isFinite(homeId) ? homeId : undefined}
        draftId={draftId}
      />
    </div>
  )
