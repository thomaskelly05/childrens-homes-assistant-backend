...
export default async function ChronologyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!event) {
    return (
      <FullScreenWorkspace
        ...
        description="This chronology event was not returned by the live chronology service. Use the chronology workspace to select another event."
        backHref="/chronology"
      >
        <Card>
  ...
  const [actionsResult, evidenceResult, peopleResult, liveContext] = await Promise.all([
    getOsActions(),
    getOsEvidence(),
    getOsYoungPeople(),
    event.youngPersonIds[0] ? getOsChronology({ youngPersonId: event.youngPersonIds[0] }) : Promise.resolve(null)
  ])
  const evidence = evidenceResult.data.filter((item) => event.evidenceIds.includes(item.id) || (item.sourceId && [event.id, event.sourceId].includes(item.sourceId)))
  const actions = actionsResult.data.filter((action) => event.actionIds.includes(action.id) || (action.sourceId && [event.id, event.sourceId].includes(action.sourceId)))
  const peopleById = new Map(peopleResult.data.map((person) => [person.id, person.displayName || person.preferredName || `Child ${person.id}`]))
  const linkedPeople = event.youngPersonIds.map((personId) => ({ id: personId, name: peopleById.get(personId) || `Child ${personId}` }))
  const linkedStaff = event.staffIds.map((staffId) => ({ id: staffId, name: `Staff ${staffId}` }))
  const context = (liveContext?.data || []).filter((item) => item.id !== event.id && item.youngPersonIds.some((personId) => event.youngPersonIds.includes(personId))).slice(0, 4)
    ...
    >
        ...
        <Card>
          ...
          <div className="mt-6 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 text-sm leading-7 text-slate-600">
              ...
              {linkedPeople.length ? linkedPeople.map((person) => <Link key={person.id} href={`/young-people/${person.id}`} className="font-black text-blue-700">{person.name} </Link>) : 'Not linked'}
            </div>
            <div className="rounded-2xl border border-slate-100 bg-slate-50 p-4 text-sm leading-7 text-slate-600">
              <strong className="text-slate-950">Linked staff:</strong> {linkedStaff.length ? linkedStaff.map((staff) => <Link key={staff.id} href={`/staff/${staff.id}`} className="font-black text-blue-700">{staff.name} </Link>) : 'Not linked'}
            </div>
          </div>
          <div className="mt-6">
            <SectionHeader eyebrow="Traceability" title="Source citations" />
            <CitationList citations={[{ label: event.citationLabel, href: `/chronology/${event.id}`, sourceType: event.sourceType, sourceId: event.sourceId, sourceDate: new Date(event.dateTime).toLocaleDateString('en-GB'), staffName: linkedStaff.map((staff) => staff.name).join(', '), youngPersonName: linkedPeople.map((person) => person.name).join(', '), confidence: event.regulationLinks.some((link) => link.confidence === 'direct') ? 'direct record' : 'supporting record', reviewRequired: event.tags.includes('manager-review') || event.tags.includes('overdue-manager-review'), excerpt: event.summary }]} />
          </div>
        </Card>
      ...
      <Card>
        ...
        <div className="grid gap-3 md:grid-cols-2">
          ...
          {!context.length ? <p className="rounded-2xl border border-slate-100 bg-slate-50 p-4 text-sm font-bold leading-6 text-slate-500">No related live chronology events were returned for this child.</p> : null}
        </div>
      </Card>
    </FullScreenWorkspace>
  )
