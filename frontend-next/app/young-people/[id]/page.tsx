  ...
  return (
    <WorkspaceStack>
      ...
      <section className="grid gap-6 xl:grid-cols-[minmax(0,1.25fr)_minmax(340px,0.75fr)]">
        <ContextSurface>
          <SectionHeader eyebrow="Person first" title="What helps this child" description="Risk is visible, but identity, strengths and communication lead the page." />
          <dl className="grid gap-3 md:grid-cols-2">
            {[
              ...
              ['Key worker', text(keyWorker, ['full_name', 'display_name', 'email'], 'Not returned yet')]
            ].map(([label, value]) => (
      ...
      <section className="grid gap-6 xl:grid-cols-2">
        <ContextSurface>
          <SectionHeader eyebrow="Relationships" title="Important people" />
          <DataTable
        ...
        <ContextSurface>
          <SectionHeader eyebrow="Plans and documents" title="Support plans and evidence" />
          <DataTable
            headers={['Record', 'Source', 'Status']}
            rows={bundle.plans.concat(bundle.documents).slice(0, 8).map((row) => [
      ...
      <ChronologySurface description="Recent child chronology from the profile bundle.">
