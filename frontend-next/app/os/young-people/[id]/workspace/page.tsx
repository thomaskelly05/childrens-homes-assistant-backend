...
export default async function OsYoungPersonWorkspacePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <>
      <SyncChildScope childId={id} childName={person?.displayName} homeId={person?.homeId} />
      <RecordWorkspacePage
        entityType="young_person"
        backHref="/select-scope"
        record={{
          id,
