...
export function OrbWriteStation({
  ...
  return (
    <OrbAppModal
      open={open}
      title="ORB Write"
      subtitle="IndiCare Intelligence's care documentation studio"
      onClose={onClose}
      panelId="orb-write"
      size="xlarge"
    ...
    >
      ...
      >
        <header className="orb-write-studio-header flex shrink-0 flex-wrap items-center gap-3" data-orb-write-studio-header>
          {onBack ? (
          ...
          <div className="min-w-0 flex-1">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-[var(--orb-muted)]">
              ORB Write
            </p>
            <h2 className="text-base font-semibold tracking-tight text-[var(--orb-foreground)]" data-orb-write-studio-title>
              IndiCare Intelligence&apos;s care documentation studio
            </h2>
            <p className="mt-0.5 text-xs leading-relaxed text-[var(--orb-muted)]" data-orb-write-studio-subtitle>
            ...
            <input
              ...
              className="mt-2 w-full bg-transparent text-lg font-semibold text-[var(--orb-foreground)] focus:outline-none"
            />
          </div>
          <div className="orb-write-studio-header__controls flex flex-wrap items-center gap-2" data-orb-write-studio-controls>
            ...
            />
            <span
              className="inline-flex rounded-full border border-amber-300/50 bg-amber-50 px-2.5 py-1 text-[11px] font-semibold text-amber-900"
              data-orb-write-review-status
            >
              {doc.is_finalised ? 'Adult approved' : 'Review required'}
            </span>
            <button
              type="button"
              className="rounded-xl bg-[var(--orb-primary,#2563eb)] px-3 py-2 text-xs font-semibold text-white"
              onClick={handleApprove}
              data-orb-write-create-final-draft
            >
              Create final draft
            </button>
          </div>
        ...
        <div className="grid min-h-0 flex-1 gap-3 lg:grid-cols-[minmax(0,1fr)_360px]" data-orb-write-layout="studio-wide">
          <OrbWriteEditor
            document={doc}
            onChange={updateBody}
