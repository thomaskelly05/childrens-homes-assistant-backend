...
export function OrbWriteTemplatePicker({
  ...
  return (
    ...
    >
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-[var(--orb-line)] bg-[var(--orb-surface)] shadow-xl">
        <header className="flex items-center justify-between border-b border-[var(--orb-line)]/50 px-4 py-3">
          ...
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1 text-[var(--orb-muted)] hover:bg-[var(--orb-surface-hover)]"
            aria-label="Close"
          >
            <X className="h-4 w-4" />
          </button>
        ...
        <div className="border-b border-[var(--orb-line)]/40 px-4 py-2">
          ...
          <div className="mt-2 flex flex-wrap gap-1" data-orb-write-template-groups>
        ...
        <div className="grid min-h-0 flex-1 gap-3 overflow-hidden p-4 sm:grid-cols-2">
          <ul className="min-h-0 space-y-2 overflow-y-auto" data-orb-write-template-list>
            {filtered.map((row) => (
              <li key={row.id}>
                <button
                  type="button"
          ...
          <div className="min-h-0 overflow-y-auto space-y-3" data-orb-write-template-detail>
            {selected ? (
              <>
                <div>
                  <p className="text-xs font-semibold text-[var(--orb-foreground)]">{selected.label}</p>
                  <p className="mt-1 text-[10px] text-[var(--orb-muted)]">{selected.writing_framework?.writing_guidance ?? selected.professional_language_guidance}</p>
                </div>
        ...
        {pendingMode ? (
          <div className="border-t border-amber-400/30 bg-amber-500/10 px-4 py-3" data-orb-write-template-replace-confirm>
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <button
                ...
                className="rounded-lg bg-amber-600 px-3 py-1.5 text-[10px] font-semibold text-white"
              ...
              <button
                ...
                className="rounded-lg px-3 py-1.5 text-[10px] text-[var(--orb-muted)]"
              >
                Cancel
              </button>
            </div>
        ...
        <footer className="flex flex-wrap gap-2 border-t border-[var(--orb-line)]/50 px-4 py-3">
          <button
            type="button"
            disabled={!selected}
            onClick={() => requestApply('full')}
            className="inline-flex items-center gap-1 rounded-lg bg-[var(--orb-primary)] px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50"
