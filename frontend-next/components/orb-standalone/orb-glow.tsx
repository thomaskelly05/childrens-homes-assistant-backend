...
export function OrbGlow({
  ...
  return (
    <div className="flex flex-col items-center text-center" aria-live="polite" aria-atomic="true">
      ...
      {compactLabels ? (
        <p className="mt-1.5 max-w-[6.5rem] text-center text-[9px] font-medium leading-tight text-[var(--orb-muted)]" data-orb-companion-label>
          {statusText}
        </p>
      ) : (
        <p className="mt-4 text-sm font-medium text-[var(--orb-foreground)]">{statusText}</p>
      )}
      {mode && !compactLabels ? (
        <p className="mt-2 text-xs font-medium text-[var(--orb-muted)]">{mode}</p>
      ) : null}
    </div>
  )
}
