...
export function OrbStandaloneSidebar() {
  return (
    <aside className="rounded-[32px] border border-white/10 bg-black/25 p-4 text-white shadow-2xl shadow-cyan-950/20 backdrop-blur">
      ...
      <nav className="mt-4 grid gap-2" aria-label="ORB standalone modes">
        {standaloneOrbBrains.map((brain) => (
          <Link key={brain} href={brain === 'Voice Assistant' ? '/assistant/voice' : '/assistant'} className="rounded-2xl bg-white/5 px-4 py-3 text-left text-sm font-black text-slate-200 hover:bg-white/10">
            {brain}
          </Link>
        ))}
      </nav>
