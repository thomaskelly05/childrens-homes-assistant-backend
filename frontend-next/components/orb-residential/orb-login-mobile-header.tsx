...
export function OrbLoginMobileHeader() {
  return (
    <header className="orb-login-mobile-header mb-4 space-y-3 text-left lg:hidden" data-orb-login-mobile-brand data-orb-login-mobile-layout data-orb-login-entrance>
      ...
      <p className="text-sm leading-relaxed text-[var(--orb-muted)]" data-orb-login-supporting>
        {ORB_LOGIN_ENTERPRISE_SUPPORTING}
      </p>

      <div className="orb-login-capability-grid space-y-2" data-orb-login-capability-groups>
        {ORB_LOGIN_CAPABILITY_GROUPS.map((group) => (
          <article key={group.id} className="orb-login-capability" data-orb-login-capability={group.id}>
            <p className="text-sm font-semibold text-[var(--orb-foreground)]">{group.label}</p>
            <p className="mt-0.5 text-xs leading-relaxed text-[var(--orb-muted)]">{group.description}</p>
          </article>
        ))}
      </div>
      ...
      <p className="text-sm" data-orb-login-demo-route>
        Not using ORB yet?{' '}
