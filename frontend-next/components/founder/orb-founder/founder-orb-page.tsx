...
import { ArrowLeft, Bot, LayoutDashboard, Lock, Sun } from 'lucide-react'
...
export function FounderOrbPage() {
  ...
  const sendQuestion = useCallback(
    (question: string, agentId?: AgentId) => {
      ...
      const answer = answerFounderQuestion(trimmed, agentId ? { agentId } : undefined)
  ...
  useEffect(() => {
    ...
    if (isValidAgentId(agentParam ?? '')) {
      ...
      setInput(question)
      sendQuestion(question, agentParam as AgentId)
      setHasAutoSent(true)
      return
    }
          ...
          <div className="flex flex-wrap items-start justify-between gap-6">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-base leading-7 text-slate-400">
                Private founder intelligence assistant for IndiCare Intelligence
              </p>
            </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[280px_minmax(0,1fr)_300px]">
          <aside className="space-y-4">
            <section className="founder-surface rounded-[28px] border border-white/10 bg-white/[0.04] p-4 backdrop-blur-xl">
              <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-slate-500">Suggested questions</p>
              <div className="mt-3 space-y-2">
                {FOUNDER_ORB_SUGGESTED_QUESTIONS.map((question) => (
                ...
                {agentShortcuts.map((agent) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      sendQuestion(`What are the latest recommendations from the ${agent.name}?`, agent.id)
                    }}
