...
export function OrbAccessibilityPanel() {
  ...
  return (
    <section className="orb-floating-panel p-6 text-white">
      <p className="orb-kicker text-[11px] font-black uppercase tracking-[0.24em]">Accessibility-first ORB</p>
      <h2 className="orb-title-glow mt-2 text-3xl font-black tracking-[-0.06em]">Make ORB easier to use</h2>
      ...
      <div className="mt-6 grid gap-3 md:grid-cols-2">
        {controls.map((control) => (
          <label key={control.key} className="orb-prompt-card flex items-center justify-between gap-4 rounded-2xl px-4 py-3 text-sm font-bold">
            {control.label}
            <input
              type="checkbox"
