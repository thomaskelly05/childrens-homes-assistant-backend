...
test.describe('ORB cookie and session E2E', () => {
  ...
  test('logout clears session and revisiting /orb shows login', async ({ page }) => {
    ...
    await expect(page.locator('[data-orb-login-signin-title]')).toContainText(/Sign in to continue/i, {
      timeout: 20_000
    })
  ...
  test('authenticated trial user sees safety acceptance and can continue to ORB', async ({ page }) => {
    await disableE2eAutoAuth(page)
    await setupOrbE2eMocks(page)
    ...
    await page.route('**/orb/standalone/safety/accept**', async (route) => {
      safetyAccepted = true
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
      ...
      const body = safetyAccepted
        ? {
            ...
            data: {
              ...
              access_blocker: null,
              safety_accepted: true,
              subscription: { active: false, status: 'trialing', plan_name: 'ORB Residential' },
              user: { id: 9001, email: E2E_CREDENTIALS.email, role: 'manager' },
              frontend_should_mount_product: true,
              allowed_bootstrap: true,
              backend_build: 'e2e-auth',
              ...
              access: {
                ...
                trial: { available: false, active: true, days_left: 14, expires_at: null },
                subscription: { active: false, status: 'trialing', plan_name: 'ORB Residential' },
                billing: { stripe_configured: true, price_gbp_monthly: 9.99 },
                standalone: true,
                os_records_accessed: false,
                ...
                upgrade: { checkout_available: true, trial_available: false, manage_billing_available: false }
              }
            }
          }
        : {
    ...
    await page.goto('/orb')
    await expect(page.locator('[data-orb-safety-acceptance]')).toBeVisible({ timeout: 20_000 })
    await expect(page.locator('[data-orb-safety-accept]')).toBeDisabled()

    for (let index = 1; index <= 4; index += 1) {
      await page.locator(`[data-orb-safety-checkbox="${index}"]`).check()
    }

    await expect(page.locator('[data-orb-safety-accept]')).toBeEnabled()
    await page.locator('[data-orb-safety-accept]').click()
    await expect(page.locator('[data-orb-shell="true"]')).toBeVisible({ timeout: 20_000 })
    await expect(page.locator('[data-orb-safety-acceptance]')).toHaveCount(0)
  })
})
