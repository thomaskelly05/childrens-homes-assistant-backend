...
def route_for_category(category: str) -> str:
    mapping = {
        ...
        'health_plan': 'health',
        'medication_document': 'health',
        'therapy_report': 'health',
        'care_plan': 'care_plan',
        'placement_plan': 'placement',
        'risk_assessment': 'risk',
        'support_plan': 'care_plan',
        'lac_review': 'care_plan',
        'pathway_plan': 'care_plan',
        'court_order': 'legal',
        'safeguarding_document': 'safeguarding',
        'missing_protocol': 'safeguarding',
        'consent_form': 'legal',
        'staff_training': 'staff_record',
        'staff_supervision': 'staff_record',
        'safer_recruitment': 'staff_record',
        'policy': 'home_compliance',
        'home_certificate': 'home_compliance',
        'insurance': 'home_compliance',
        'reg44': 'reg44',
        'reg45': 'reg45',
...
def parse_dates(text: str) -> dict[str, datetime.date | None]:
    ...
    if any(word in lower for word in ['expiry', 'expires', 'valid until', 'renewal due']):
        expiry = future_dates[0] if future_dates else None
    if any(word in lower for word in ['review date', 'next review', 'review due', 'pep review', 'annual review']):
        review = future_dates[0] if future_dates else None
    if not expiry and not review and future_dates:
        review = future_dates[0]
    return {
        'document_date': past_dates[0] if past_dates else None,
        'effective_date': past_dates[0] if past_dates else None,
        'expiry_date': expiry,
        'review_date': review,
        'next_action_date': expiry or review,
    }


def make_summary(text: str, category: str) -> dict[str, str | None]:
    preview = ' '.join((text or '').split())[:900]
    education = preview if category in {'pep', 'ehcp', 'education_plan'} else None
    health = preview if category in {'health_plan', 'medication_document', 'therapy_report'} else None
    placement = preview if category in {'placement_plan', 'care_plan', 'support_plan'} else None
    safeguarding = preview if category in {'risk_assessment', 'safeguarding_document', 'missing_protocol'} else None
    compliance = preview if category in {'policy', 'insurance', 'reg44', 'reg45', 'inspection_report', 'home_certificate'} else None
    return {
        ...
        'safeguarding_summary': safeguarding,
        'compliance_summary': compliance,
    }


def create_route(cursor: Any, job_id: str, category: str, route_target: str, title: str, summary: str | None, review: dict[str, Any]) -> str:
    cursor.execute(
        '''
        INSERT INTO public.document_intelligence_routes (
          job_id, route_target, target_table, route_title, route_summary, extracted_payload
        ) VALUES (%s::uuid,%s,%s,%s,%s,%s::jsonb)
        RETURNING id
        ''',
        (job_id, route_target, route_target, title, summary, {'category': category, 'review': review}),
    )
    row = cursor.fetchone()
...
def create_reminders(cursor: Any, job: dict[str, Any], actor_id: int | None, create_tasks: bool = True) -> list[str]:
    ...
    for date_field, reminder_type in [('expiry_date', 'document_expiry'), ('review_date', 'document_review')]:
        ...
        reminder_date = target_date - datetime.timedelta(days=30)
        if reminder_date < datetime.date.today():
            reminder_date = datetime.date.today()
        cursor.execute(
            '''
            INSERT INTO public.document_expiry_reminders (
              job_id, attachment_id, provider_id, home_id, young_person_id, staff_id, adult_id,
              reminder_type, title, description, reminder_date, due_date, priority, created_by
            ) VALUES (%s::uuid,%s::uuid,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
            RETURNING id
            ''',
            (
                job['id'], job.get('attachment_id'), job.get('provider_id'), job.get('home_id'), job.get('young_person_id'), job.get('staff_id'), job.get('adult_id'),
                reminder_type,
                f"Review {job.get('detected_title') or job.get('file_name') or 'document'}",
                f"Document is due for {'expiry' if reminder_type == 'document_expiry' else 'review'} on {target_date.isoformat()}.",
                reminder_date,
                target_date,
                'high' if reminder_type == 'document_expiry' else 'normal',
                actor_id,
            ),
        )
        row = cursor.fetchone()
...
async def analyse_document(payload: AnalyseDocumentRequest, request: Request):
    ...
    try:
        ...
        with conn.cursor() as cursor:
            ...
            existing_review = review_document(
                document_type=category,
                payload={'file_name': file_name, 'source_text': source_text, **{k: v for k, v in dates.items() if v}},
                actions=['classify', 'route', 'expiry_reminders'],
            )
            ...
            cursor.execute(
                ...
                ''',
                (
                    payload.attachment_id, payload.source_record_id, payload.provider_id, payload.home_id, payload.young_person_id, payload.staff_id, payload.adult_id,
                    file_name, mime_type, source_text, category, detected_title,
                    dates['document_date'], dates['effective_date'], dates['expiry_date'], dates['review_date'], dates['next_action_date'],
                    summaries['summary'], summaries['key_information'], None, None,
                    summaries['safeguarding_summary'], summaries['education_summary'], summaries['health_summary'], summaries['placement_summary'], summaries['compliance_summary'],
                    0.70 if not requires_review else 0.35, requires_review, review_reason, actor_id,
                    {**payload.metadata, 'existing_document_ai_review': existing_review, 'route_target': route_target},
                ),
            )
            job = rows_to_dicts(cursor, cursor.fetchall())[0]
            route_id = create_route(cursor, job['id'], category, route_target, detected_title, summaries['summary'], existing_review)
            ...
            cursor.execute(
                '''
                INSERT INTO public.document_intelligence_audit_events (job_id, route_id, event_type, event_summary, actor_id, after_snapshot)
                VALUES (%s::uuid,%s::uuid,'document_analysed','Document analysed, classified and routed',%s,%s::jsonb)
                ''',
                (job['id'], route_id, actor_id, {'category': category, 'route_target': route_target, 'reminders': reminders}),
            )
            conn.commit()
            return {'job': job, 'route_id': route_id, 'reminders': reminders, 'review': existing_review, 'status': status}
...
async def mark_routed(route_id: str, payload: RouteDocumentRequest, request: Request):
    ...
    try:
        ...
        with conn.cursor() as cursor:
            cursor.execute(
                'UPDATE public.document_intelligence_routes SET routed = true, routed_by = %s, routed_at = now() WHERE id::text = %s RETURNING *',
                (actor_id, route_id),
            )
            rows = rows_to_dicts(cursor, cursor.fetchall())
            if not rows:
