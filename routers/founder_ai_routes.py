...
from services.founder_ai_service import normalise_founder_mode, run_founder_ai
...
def require_founder_user(current_user: dict[str, Any]) -> dict[str, Any]:
...
async def founder_health(
    ...
    require_founder_user(current_user)
    return {"ok": True, "area": "founder_hq"}
...
async def founder_threads(
    ...
    return {"threads": await list_founder_threads(user_id)}
...
async def founder_thread_messages(
    ...
    user_id = get_user_id(current_user)
    return {"messages": await get_founder_messages(user_id, thread_id)}


@router.post("/ai/chat", response_model=FounderChatResponse)
