        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                WHERE dn.young_person_id = %s
                  ...
                  AND LOWER(COALESCE(dn.workflow_status, 'draft')) NOT IN ('reviewed', 'archived')
...
def get_young_person_daily_notes_archive(young_person_id: int, conn=Depends(get_db)):
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT id FROM young_people WHERE id = %s LIMIT 1", (young_person_id,))
            if not cur.fetchone():
            ...
            cur.execute(
                ...
                WHERE dn.young_person_id = %s
                  AND (
                    COALESCE(dn.archived, FALSE) = TRUE
                    OR LOWER(COALESCE(dn.workflow_status, '')) IN ('reviewed', 'archived')
                  )
