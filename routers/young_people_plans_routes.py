    ...
    try:
        with conn.cursor() as cur:
            ...
            cur.execute(
                ...
                LEFT JOIN users ou
                    ON sp.owner_id = ou.id
                LEFT JOIN users cu
                    ON sp.created_by = cu.id
...
def get_support_plan(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to load support plan: {str(e)}"
        )
...
def create_support_plan(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to create support plan: {str(e)}"
        )
...
def update_support_plan(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to update support plan: {str(e)}"
        )
