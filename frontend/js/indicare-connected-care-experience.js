(() => {
  ...
  function ensureLauncher() {
    ...
    root.innerHTML = `
      <div class="care-launcher">
        <button class="care-launcher-btn" type="button" data-care-open title="Alt+R">+ Record</button>
        <button class="care-launcher-mini" type="button" data-care-assistant title="Alt+I">Ask IndiCare</button>
      </div>
      <section class="care-launcher-panel" data-care-panel>
        ...
        <div class="care-shortcuts"><button class="care-chip" data-care-shortcut="daily_note">Daily</button><button class="care-chip" data-care-shortcut="incident">Incident</button><button class="care-chip" data-care-shortcut="keywork">Key work</button><button class="care-chip" data-care-shortcut="ask">Ask</button></div>
        <input class="care-search" data-care-search placeholder="Search actions: daily, incident, document, review..." />
  ...
  function togglePanel() {
    ...
    document.querySelector('[data-care-search]')?.focus?.();
  }
  ...
  function assistantContextPrompt(prefix = '') {
    const c = currentContext();
    const base = c.young_person_id
      ? `Current context: child=${c.child_name || 'selected child'}, home=${c.home_id}, page=${c.page}, records_today=${c.records_today}, manager_reviews=${c.manager_reviews}, open_actions=${c.open_commands}, disruption_risk=${c.disruption_risk}.`
      : `Current context: home=${c.home_id}, page=${c.page}.`;
    return `${base}\n${prefix}`.trim();
  }

  function askAssistantAboutContext() {
    window.IndiCareOSAssistant?.open?.();
    window.IndiCareOSAssistant?.ask?.(assistantContextPrompt('Summarise current priorities, open callbacks, recent chronology and what staff should be mindful of when recording today.'));
  }
  ...
  function emitCareEvent(name, detail = {}) { document.dispatchEvent(new CustomEvent(name, { detail: { ...detail, context: currentContext() } })); }
  ...
  function patchRefreshEvents() {
    ...
    document.addEventListener('indicare:document-intelligence-complete', (event) => {
      ...
      showToast('Document connected', 'The document was analysed and routed into the connected care record system.', [
        { label: 'Ask assistant', primary: true, run: () => window.IndiCareOSAssistant?.ask?.(assistantContextPrompt('Explain the latest document routing and what it changes for this child or home.')) },
      ]);
    });
    document.addEventListener('indicare:care-data-changed', () => {
      ...
      if (typeof window.loadAll === 'function') { try { window.loadAll(); } catch {} }
    });
  }
  ...
  function patchFetchForRecordSaves() {
    ...
    window.fetch = async function connectedCareFetch(...args) {
      ...
      try {
        ...
        if (['POST','PATCH','PUT'].includes(method) && /daily-notes|incidents|keywork|universal-records|document-intelligence|reg44-reader|tasks|actions|chronology|handover/i.test(url)) {
  ...
  function exposeGlobalApi() {
    window.IndiCareConnectedCare = {
      ...
      assistantPrompt: assistantContextPrompt,
