(() => {
  ...
  function buildDashboard() {
    ...
    const wellbeing = children.filter(c => /anxious|distress|dysregulat/i.test(safeStringify(c))).length;
    const missing = children.filter(c => /missing/i.test(safeStringify(c))).length;
  ...
  function bind(root) {
    root.querySelectorAll('[data-over-ask]').forEach(btn => {
  ...
  function boot() {
    ...
    window[FLAG] = true;
    injectStyles();
    setTimeout(() => window.IndiCareSafe?.run?.('oversight mount', mount) || mount(), 250);
    document.addEventListener('indicare:care-data-changed', () => {
      window.IndiCareSafe?.debounce?.('oversight-remount', mount, 400);
    });
    window.IndiCareOversightDashboard = { mount };
  }
