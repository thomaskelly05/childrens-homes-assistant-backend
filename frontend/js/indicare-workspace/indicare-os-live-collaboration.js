import { getOsContext, getOperationalSession, scopeContextToSession, escapeHtml, formatDate, childName, childKey, recordType, displayType } from './indicare-os-context.js';
...
function bootLiveCollaboration() {
  ...
  const observer = new MutationObserver(() => renderDashboardCollaboration());
...
function hydrateLocalState() {
  try { LIVE_STATE.messages = JSON.parse(localStorage.getItem('indicare.os.live.messages') || '[]'); } catch { LIVE_STATE.messages = []; }
}

function persistMessages() {
  localStorage.setItem('indicare.os.live.messages', JSON.stringify(LIVE_STATE.messages.slice(0, 100)));
}
...
function rebuildPresence() {
  ...
  LIVE_STATE.presence = staff.length ? dedupe(staff).slice(0, 10).map((person, index) => ({
    ...
    status: person.status || person.availability || (index === 0 ? 'Reviewing' : 'Online'),
    room: index % 3 === 0 ? 'safeguarding' : index % 2 === 0 ? 'review' : 'shift',
    active: true,
  })) : [{ id: 'current', name: 'Current user', role: 'IndiCare OS user', status: 'Online', room: 'shift', active: true }, { id: 'lead', name: 'Shift Lead', role: 'Residential Team Leader', status: 'Available', room: 'review', active: true }];
}
...
function renderDashboardCollaboration({ force = false } = {}) {
  ...
  const html = `
    <section class="sp-card os-live-collab-card" data-os-live-collab>
      <div class="sp-card-head">
        <div><h2>Live collaboration</h2><p>Shift rooms, safeguarding escalations, review conversations and staff presence.</p></div>
        <button type="button" data-open-live-collaboration>Open collaboration →</button>
      </div>
      <div class="os-live-grid">
        <article class="os-live-presence"><h3>Presence</h3>${presenceList()}</article>
        <article class="os-live-summary"><h3>Operational rooms</h3><div class="os-room-metrics"><button type="button" data-open-live-room="shift"><strong>${roomMessages('shift').length}</strong><span>Shift messages</span></button><button type="button" data-open-live-room="safeguarding"><strong>${escalationCount}</strong><span>Safeguarding</span></button><button type="button" data-open-live-room="review"><strong>${reviewCount}</strong><span>Review desk</span></button></div></article>
        <article class="os-live-latest"><h3>Latest discussion</h3>${latest.length ? latest.map(messageCard).join('') : emptyMini('No collaboration messages yet. Start with a shift update or safeguarding note.')}</article>
      </div>
    </section>`;
...
function handleKeys(event) {
  const form = event.target.closest?.('[data-live-compose]');
...
function submitMessage(form) {
  ...
  if (!text) return;
  LIVE_STATE.messages.unshift({ id: crypto?.randomUUID?.() || String(Date.now()), room: LIVE_STATE.activeRoom, author: 'Current user', role: 'Operational user', text, createdAt: new Date().toISOString() });
  persistMessages();
...
function fillTemplate(type) {
  const input = document.querySelector('#ic-os-live-collab-panel [data-live-compose] textarea');
  if (!input) return;
  input.value = type === 'safeguarding' ? 'Safeguarding escalation:\nYoung person:\nConcern:\nImmediate safety actions:\nWho has been informed:\nNext review/action:' : 'Handover update:\nYoung person/home area:\nWhat happened:\nRisk/need:\nAction for next shift:\nManager oversight required:';
  input.focus();
}

function seedRecordDiscussion(record) {
  if (!record) return;
  const type = /safeguarding|missing|incident|risk/i.test(recordType(record)) ? 'safeguarding' : /review|submitted|pending|changes/i.test(`${record.status || ''} ${record.workflow_status || ''}`) ? 'review' : 'shift';
  LIVE_STATE.activeRoom = type;
}

function presenceList() {
  return `<div class="os-presence-list">${LIVE_STATE.presence.slice(0, 6).map((person) => `<p><span class="os-presence-dot"></span><strong>${escapeHtml(person.name)}</strong><small>${escapeHtml(person.role)} · ${escapeHtml(person.status)}</small></p>`).join('')}</div>`;
}

function messageCard(message) {
  return `<article class="os-live-message"><div><strong>${escapeHtml(message.author)}</strong><span>${escapeHtml(formatDate(message.createdAt))}</span></div><p>${escapeHtml(message.text)}</p></article>`;
}
...
function dedupe(items) { const seen = new Set(); return items.filter((item) => { const key = item.id || item.user_id || item.email || item.name || item.full_name; if (seen.has(key)) return false; seen.add(key); return true; }); }
