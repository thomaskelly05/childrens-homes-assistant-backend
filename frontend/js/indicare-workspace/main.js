...
function bindEvents() {
  els.nav?.addEventListener("click", (e) => { if (!e.target.matches("button")) return; $$(".nav-item").forEach((b) => b.classList.remove("active")); e.target.classList.add("active"); loadView(e.target.dataset.view); });
...
function renderChild(data) { const child = data.child || data.children?.[0] || {}; const journey = data.journey || data.child_journey_overview || {}; els.main.innerHTML = `${data.error ? warning(data.error) : ""}<section class="hero-card child-first"><div><p class="eyebrow">Identity first</p><h3>${esc(fullName(child) || "Selected child")}</h3><p>Start with safety, strengths, communication, voice and what adults need to understand.</p></div></section><section class="card-grid">${metric("Timeline items", journey.timeline?.length || journey.recent_events?.length || 0, "Journey evidence")}${metric("Actions", data.adult_workspace?.actions?.length || 0, "Adult follow-through")}${metric("Documents", data.documents?.length || 0, "Plans and statutory evidence")}${metric("Review queue", data.manager_oversight?.review_queue?.length || 0, "Manager grip")}</section><section class="two-column">${panel("What helps me", `<div class="identity-grid"><div>What helps me feel safe</div><div>What I want adults to know</div><div>My strengths</div><div>How I communicate distress</div></div>`)}${panel("Recent journey", timeline(journey.timeline || journey.recent_events || []))}</section>`; }
...
function reviewCard(item) { const type = item.record_type || item.source_table || "Record"; const priority = item.priority || "medium"; return `<article class="record-card review-card"><div><span class="mini-tag">${esc(priority)}</span><h4>${esc(type)}</h4><p>${esc(item.summary || "Record waiting for manager review")}</p><small>Source: ${esc(item.source_table || "unknown")} #${esc(item.source_id || "")}</small></div><div class="record-actions"><button onclick="reviewAction(${Number(item.id)}, 'approve')">Approve</button><button onclick="reviewAction(${Number(item.id)}, 'return')">Return</button><button onclick="reviewAction(${Number(item.id)}, 'acknowledge')">Acknowledge</button></div></article>`; }
...
function timeline(items) { return `<ol class="timeline">${(items || []).slice(0, 12).map((i) => `<li><strong>${esc(i.title || i.record_type || "Record")}</strong><span>${esc(i.summary || "No summary recorded")}</span></li>`).join("") || "<li>No journey events loaded.</li>"}</ol>`; }
