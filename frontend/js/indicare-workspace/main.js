...
function renderHome(data) { const counts = data.child_journey_overview?.counts || {}; const homeEvents = data.child_journey_overview?.recent_events || []; const alerts = generateAlerts(homeEvents, data.manager_oversight?.review_queue || []); els.main.innerHTML = `${data.error ? warning(data.error) : ""}<section class="hero-card"><div><p class="eyebrow">Home overview</p><h3>${esc(data.home?.name || "Children's home")}</h3><p>Live view of children, staff tasks, documents, risk and Ofsted readiness.</p></div><span class="score-pill">${alerts.length} alert(s)</span></section><section class="card-grid">${metric("Children", data.children?.length || 0, "Currently visible in placement")}${metric("Daily notes", counts.daily_note || 0, "Recent lived experience evidence")}${metric("Incidents", counts.incident || 0, "Require pattern review")}${metric("Safeguarding", counts.safeguarding || 0, "Must show oversight")}</section><section class="two-column">${panel("Risk and alerts", renderAlerts(alerts))}${panel("Manager oversight", list(["Review queue: " + (data.manager_oversight?.review_queue?.length || 0), "Open actions: " + (data.manager_oversight?.open_or_overdue_actions?.length || 0), "Ofsted readiness visible"]))}</section>`; }
...
function renderChild(data) {
  ...
  const alerts = generateAlerts(events, data.manager_oversight?.review_queue || []);
  els.main.innerHTML = `${data.error ? warning(data.error) : ""}
    ...
    <section class="card-grid">${metric("Timeline items", events.length, "Journey evidence")}${metric("Incidents", counts.incident || counts.incident_record || 0, "Pattern review")}${metric("Safeguarding", counts.safeguarding || 0, "Protection evidence")}${metric("Alerts", alerts.length, "Risk and practice signals")}</section>
    <section class="two-column">${panel("Risk and alerts", renderAlerts(alerts))}${panel("Journey insights", journeyInsightCards(events))}</section>
    <section class="two-column">${panel("What helps me", `<div class="identity-grid"><div>What helps me feel safe</div><div>What I want adults to know</div><div>My strengths</div><div>How I communicate distress</div></div>`)}${panel("Manager oversight", list(["Review queue: " + (data.manager_oversight?.review_queue?.length || 0), "Documents: " + (data.documents?.length || 0), "Actions: " + (data.adult_workspace?.actions?.length || 0)]))}</section>
    <section class="panel"><h3>Child journey timeline</h3>${timeline(events, true)}</section>`;
}

function generateAlerts(events = [], reviewQueue = []) {
  const alerts = [];
  const counts = countEvents(events);
  const incidentCount = (counts.incident || counts.incident_record || 0);
  const safeguardingCount = counts.safeguarding || counts.safeguarding_records || 0;
  const missingCount = counts.missing_episode || counts.missing_episodes || 0;
  const keyworkCount = counts.keywork || counts.keywork_sessions || 0;
  const pendingSafeguarding = (reviewQueue || []).filter((item) => `${item.record_type || ""} ${item.source_table || ""} ${item.summary || ""}`.toLowerCase().includes("safeguarding"));

  if (incidentCount >= 3) alerts.push({ level: "high", message: "Multiple incidents visible", why: "This may indicate escalation in distress, unmet need or instability.", action: "Review triggers, behaviour support plan, risk assessment and staff response." });
  if (safeguardingCount > 0) alerts.push({ level: "high", message: "Safeguarding activity visible", why: "Safeguarding records must show immediate safety action, notifications, outcome and management oversight.", action: "Check review status, external notifications and actions." });
  if (pendingSafeguarding.length) alerts.push({ level: "high", message: "Safeguarding awaiting manager review", why: "Unreviewed safeguarding weakens protection evidence and leadership grip.", action: "Manager review required immediately." });
  if (missingCount >= 2) alerts.push({ level: "high", message: "Repeated missing episodes", why: "Repeated missing episodes may indicate exploitation, emotional distress or unsafe associates.", action: "Review missing plan, trigger pattern and return home interview evidence." });
  if (keyworkCount === 0 && events.length) alerts.push({ level: "medium", message: "No keywork visible", why: "This may limit evidence of child voice, wishes, feelings and relational work.", action: "Schedule and record keywork or direct work." });
  if (!alerts.length && events.length) alerts.push({ level: "low", message: "No immediate alert pattern", why: "Records are visible without obvious escalation signals in this view.", action: "Continue monitoring child voice, risk and adult response." });
  return alerts;
}

function renderAlerts(alerts = []) { if (!alerts.length) return empty("No alerts identified in this view."); return alerts.map((a) => `<article class="alert-card ${esc(a.level)}"><strong>${esc(a.message)}</strong><p>${esc(a.why)}</p><small>${esc(a.action)}</small></article>`).join(""); }
