...
function renderBundleStatus() {
  ...
  if (els.assistantScopeBundleError) {
    ...
    els.assistantScopeBundleError.classList.toggle("hidden", !state.scopeBundleError);
  }

  if (els.assistantLiveStatus) {
    const summary = state.assistantMeta?.live_summary;
    if (summary && typeof summary === "object") {
      els.assistantLiveStatus.textContent = `Live analysis ready • evidence items: ${summary.total ?? 0}`;
...
function renderBriefPanel(host, brief, emptyText = "No brief available yet.") {
  ...
  const keyConcerns = Array.isArray(brief.key_concerns) ? brief.key_concerns : [];
  const evidence = Array.isArray(brief.evidence) ? brief.evidence : [];
  const chronology = Array.isArray(brief.chronology) ? brief.chronology : [];
    ...
    <div class="profile-stack">
      <div class="profile-card">
        ...
        <div class="profile-card-text">
          Scope: ${escapeHtml(brief.scope || "unknown")} • Section: ${escapeHtml(brief.section || "unknown")}
        </div>
      </div>
      ...
      <div class="profile-card">
        ...
        <div class="profile-card-text">
          Evidence items: ${escapeHtml(String(brief.summary?.total ?? evidence.length ?? 0))}<br />
          Open tasks: ${escapeHtml(String(brief.summary?.open_tasks ?? 0))}<br />
          Overdue items: ${escapeHtml(String(brief.summary?.overdue_items ?? 0))}<br />
        ...
        keyConcerns.length
          ? `
            <div class="profile-card">
              ...
              <div class="profile-card-text">${keyConcerns.map((item) => escapeHtml(item)).join("<br />")}</div>
            </div>
          `
          : ""
      }
      ...
      ${
        facts.next_appointment || facts.latest_incident || facts.latest_missing_episode
          ? `
            <div class="profile-card">
              ...
              <div class="profile-card-text">
                ${
                  facts.next_appointment
                    ? `Next appointment: ${escapeHtml(facts.next_appointment.title || "Appointment")} ${facts.next_appointment.date ? `(${escapeHtml(String(facts.next_appointment.date))})` : ""}<br />`
                    : ""
                }
                ${
                  facts.latest_incident
                    ? `Latest incident: ${escapeHtml(facts.latest_incident.title || "Incident")} ${facts.latest_incident.date ? `(${escapeHtml(String(facts.latest_incident.date))})` : ""}<br />`
                    : ""
                }
                ${
                  facts.latest_missing_episode
                    ? `Latest missing episode: ${escapeHtml(facts.latest_missing_episode.title || "Missing episode")} ${facts.latest_missing_episode.date ? `(${escapeHtml(String(facts.latest_missing_episode.date))})` : ""}`
                    : ""
                }
              </div>
            </div>
            ...
            <div class="profile-card">
              ...
              <div class="profile-card-text">
                ${chronology
                  ...
                  .map((item) =>
                    escapeHtml(
                      [item.date, item.title, item.summary].filter(Boolean).join(" - ")
                    )
                  )
                  .join("<br />")}
              </div>
    ...
    .map((update) => {
      ...
      return `
        <div class="entity-row">
          ...
          ${message ? `<div class="entity-meta" style="margin-top:6px;">${message}</div>` : ""}
        </div>
      `;
    })
    .join("");
...
function buildDerivedAssistantStateFromBundle(bundle) {
  ...
  const morningBrief = buildMorningBriefContext({
    records_payload,
  });

  const managerBrief = buildManagerBriefContext({
    records_payload,
  });

  const qualityBrief = buildQualityBriefContext({
    records_payload,
  });
  ...
  if (state.assistantMeta) {
    ...
    state.assistantMeta.evidence_sufficiency = runtime.evidence_sufficiency || {};
...
export async function refreshAssistantScopeData({
  ...
  try {
    ...
    buildDerivedAssistantStateFromBundle(bundle);
    ...
    if (pushUpdate) {
      pushAssistantLiveUpdate({
        ...
        message:
          context.scope === "child"
            ? `Scoped records refreshed for ${context.young_person_name || "the selected young person"}.`
            : `Scoped records refreshed for ${context.home_name || "the selected home"}.`,
...
export async function refreshAssistantAnalysisOnly() {
  ...
  pushAssistantLiveUpdate({
    ...
    message: "Derived summaries, facts, chronology, and brief panels were updated.",
    created_at: new Date().toISOString(),
  });
