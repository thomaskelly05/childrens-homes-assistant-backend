...
function inferSectionForRecordType(recordType = "", scope = getCurrentScope()) {
  const map = {
    ...
    inspection_refresh: scope === "quality" ? "inspection-readiness" : "ofsted-readiness",
    inspection_sync: scope === "quality" ? "inspection-readiness" : "ofsted-readiness",
...
function buildQuickActionMap() {
  ...
  [...(CONFIG_QUICK_ACTIONS || []), ...(CONFIG_PROFILE_ACTIONS || [])].forEach(
    (action) => {
      ...
      actions[normaliseActionKey(action.id)] = {
        id: normaliseActionKey(action.id),
