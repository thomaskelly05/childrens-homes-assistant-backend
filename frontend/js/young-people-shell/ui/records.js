...
safeguarding_record: `/young-people/safeguarding-records/${id}`,
missing_episode: `/young-people/missing-episodes/${id}`,
task: `/young-people/tasks/${id}`,
};
...
return value
.map((item) => escapeHtml(typeof item === "object" ? JSON.stringify(item) : String(item)))
.join(", ");
}
...
const suggestions = mergeSuggestionLists(evaluateRecordSuggestions(suggestionRecord));
