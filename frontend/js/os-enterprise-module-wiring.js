(() => {
  ...
  const MODULES = [
    { id: 'staffing', hydration: 'staffing', label: 'Staffing & Workforce', description: 'Rotas, staffing continuity, supervision, training and workforce risk.', routes: ['/api/staff', '/api/rota', '/api/supervision', '/api/staff-today', '/api/staff-profile'], views: ['adult-profile'], prompts: ['Show staffing risks', 'Prepare staffing handover', 'Training compliance overview'] },
    { id: 'documents-dms', hydration: 'documents', label: 'Home Documents', description: 'Policies, plans, assessments, evidence and inspection documentation.', routes: ['/api/documents', '/api/document-library', '/api/os-command/inspection/workspaces', '/api/evidence'], views: ['documents', 'standards-ofsted'], prompts: ['Inspection evidence gaps', 'Reg 44 preparation', 'Document review summary'] },
    { id: 'medication', hydration: 'health', label: 'Medication & Health', description: 'MAR, medication administration, appointments, sleep and wellbeing.', routes: ['/api/mar', '/api/health', '/api/young-people/health'], views: ['health'], prompts: ['Medication risks', 'Health appointments', 'Sleep concerns overview'] },
    { id: 'compliance', hydration: 'compliance', label: 'Compliance & Oversight', description: 'Regulation, audit, quality assurance and operational compliance.', routes: ['/api/compliance', '/api/reg44', '/api/qa', '/api/reports', '/api/ofsted-pack'], views: ['standards-ofsted', 'review'], prompts: ['Compliance concerns', 'Audit summary', 'Outstanding actions'] },
    { id: 'finance', hydration: 'connect', label: 'Finance & Allowances', description: 'Pocket money, incentives, sanctions, purchases, claims and placement cost continuity.', routes: ['/api/finance', '/api/pocket-money', '/api/sanctions'], views: [], prompts: ['Allowance audit', 'Spending concerns', 'Sanctions and incentives overview'] },
    { id: 'connect', hydration: 'connect', label: 'Connect & Communications', description: 'Internal messages, multi-agency communication, meetings, calls and linked records.', routes: ['/api/connect', '/api/mail', '/api/calendar', '/api/tasks'], views: [], prompts: ['Communication summary', 'Meeting follow-up', 'Multi-agency actions'] }
  ];
      ...
      <section class="two-column">
        <article class="panel">
          <div class="section-header-row">
            ...
            <div class="hero-actions">
              ...
              ${module.views.map((view) => `<button class="ic365-button" data-launch-existing-view="${esc(view)}">Open ${esc(view)}</button>`).join('')}
            </div>
          </div>
          ...
          <div class="ic365-continuity-grid">
            <article class="ic365-continuity-panel">
              ...
              <div class="ic365-continuity-list">${routeCards(results)}</div>
            </article>

            <article class="ic365-continuity-panel">
              <h3>Operational hydration</h3>
              <div class="ic365-continuity-list" data-enterprise-hydration>
                <div class="ic365-empty-state">Hydrating operational data...</div>
              </div>
            </article>
          </div>
        </article>
        ...
        <article class="panel">
          ...
          <div class="ic365-action-list">
            ${module.prompts.map((prompt) => `<button type="button" data-os-assistant-prompt="${esc(prompt)}"><strong>${esc(prompt)}</strong><span>Run operational intelligence</span></button>`).join('')}
          </div>
        </article>
      </section>`;

    if (window.IndiCareEnterpriseHydration?.hydrateModule) {
      window.IndiCareEnterpriseHydration.hydrateModule(module.hydration || module.id);
    }
