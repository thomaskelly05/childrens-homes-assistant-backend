(() => {
  ...
  async function openUniversalRecord(recordId) {
    ...
    try {
      ...
      body.innerHTML = `
        <section class="record-section"><h4>Record summary</h4><p>${esc(r.summary || r.narrative || 'No summary recorded.')}</p>${r.child_voice ? `<p><strong>Child voice:</strong> ${esc(r.child_voice)}</p>` : ''}${r.staff_analysis || r.staff_reflection ? `<p><strong>Staff reflection:</strong> ${esc(r.staff_analysis || r.staff_reflection)}</p>` : ''}</section>
        ...
        <section class="record-section"><h4>Safeguarding / inspection</h4><div class="record-field-grid"><div class="record-field"><strong>Safeguarding relevant</strong>${r.safeguarding_relevant ? 'Yes' : 'No'}</div><div class="record-field"><strong>Inspection relevant</strong>${r.inspection_relevant ? 'Yes' : 'No'}</div><div class="record-field"><strong>Manager review</strong>${r.manager_review_required ? 'Required' : 'Not required'}</div><div class="record-field"><strong>Restricted</strong>${r.restricted ? 'Yes' : 'No'}</div></div></section>
  ...
  function renderChildTabs(data) {
    ...
    return `
      <div class="child-workspace-tabs" role="tablist">${TAB_DEFS.map(([key,label],i)=>`<button type="button" class="child-workspace-tab ${i===0?'active':''}" data-child-tab="${key}">${label}</button>`).join('')}</div>
      <section class="child-tab-view active" data-child-tab-view="overview"><div class="child-tab-grid"><div class="ic-metric"><strong class="ic-metric-value">${esc(p.records_today||0)}</strong><span class="ic-metric-label">Records today</span></div><div class="ic-metric"><strong class="ic-metric-value">${esc(p.manager_review_count||0)}</strong><span class="ic-metric-label">Manager reviews</span></div><div class="ic-metric"><strong class="ic-metric-value">${esc(p.open_commands||commandItems.length||0)}</strong><span class="ic-metric-label">Open commands</span></div><div class="ic-metric"><strong class="ic-metric-value">${esc(Math.round(p.disruption_risk_score||data?.placement_stability?.disruption_risk_score||0))}</strong><span class="ic-metric-label">Disruption risk</span></div></div><div class="child-tab-section-title">Latest chronology</div>${timelineRows(timeline.slice(0,6))}</section>
      <section class="child-tab-view" data-child-tab-view="journey"><div class="child-tab-section-title">Full child journey</div>${recordToolbar('journey')}</section>
      <section class="child-tab-view" data-child-tab-view="care"><div class="child-tab-section-title">Care notes and daily records</div>${recordToolbar('care')}</section>
      <section class="child-tab-view" data-child-tab-view="safeguarding"><div class="child-tab-section-title">Safeguarding records, alerts and concerns</div>${recordToolbar('safeguarding')}</section>
      <section class="child-tab-view" data-child-tab-view="plans"><div class="child-tab-section-title">Risk assessments, support plans and reviews</div>${recordToolbar('plans')}</section>
      <section class="child-tab-view" data-child-tab-view="education"><div class="child-tab-section-title">Education records</div>${recordToolbar('education')}</section>
      <section class="child-tab-view" data-child-tab-view="health"><div class="child-tab-section-title">Health and wellbeing records</div>${recordToolbar('health')}</section>
      <section class="child-tab-view" data-child-tab-view="family"><div class="child-tab-section-title">Family contact records</div>${recordToolbar('family')}</section>
      <section class="child-tab-view" data-child-tab-view="keywork"><div class="child-tab-section-title">Key work records</div>${recordToolbar('keywork')}</section>
      <section class="child-tab-view" data-child-tab-view="incidents"><div class="child-tab-section-title">Incidents and missing episodes</div>${recordToolbar('incidents')}</section>
      <section class="child-tab-view" data-child-tab-view="command"><div class="child-tab-section-title">Command items</div><div class="ic-list">${commandItems.length?commandItems.map(r=>`<div class="ic-item"><div class="ic-item-top"><div><div class="ic-item-title">${esc(r.title)}</div><div class="ic-item-text">${esc(r.summary||r.recommended_action||'')}</div></div>${badge(r.priority||r.status)}</div></div>`).join(''):empty('No command items for this child.')}</div></section>
    `;
  }
  ...
  function activateTab(root, key) {
    ...
    if (!['overview','command'].includes(key)) loadUniversalRecords(key, root);
  }

  function bindRecordFilters(root) {
    root.querySelectorAll('[data-record-search],[data-record-category],[data-record-risk]').forEach((control) => {
      const handler = () => {
        const view = control.closest('[data-child-tab-view]');
        if (view?.dataset?.childTabView) loadUniversalRecords(view.dataset.childTabView, root);
      };
      control.addEventListener(control.tagName === 'INPUT' ? 'input' : 'change', debounce(handler, 250));
    });
  }
  ...
  function enhanceCurrentChildWorkspace() {
    ...
    root.querySelectorAll('.child-workspace-tab').forEach((btn)=>btn.addEventListener('click',()=>activateTab(root, btn.dataset.childTab)));
    bindRecordFilters(root);
    return true;
  }
  ...
  function patchOpenChild() {
    ...
    window.openChild = async function patchedOpenChild(...args) { const result = await originalOpenChild.apply(this,args); enhanceCurrentChildWorkspace(); return result; };
    window[PATCH_FLAG] = true;
  }

  function boot() {
    injectStyles(); ensureDrawer(); patchOpenChild();
    const observer = new MutationObserver(()=>{ patchOpenChild(); const root=document.getElementById('child-workspace'); if(root&&window.state?.selectedChild?.profile&&!root.querySelector('.child-workspace-tabs')) enhanceCurrentChildWorkspace(); });
