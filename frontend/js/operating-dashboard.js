...
function activeSectionId() {
  ...
  return findCareHubSection(hash) ? hash : "today";
}
...
function ensureCareHubScreenShell() {
  ...
  if (!byId("opsScreenTitle")) {
    header.innerHTML = `
      <div>
        <p class="ops-eyebrow">IndiCare OS</p>
        <h1 id="opsScreenTitle">Today</h1>
        <p id="opsScreenIntro">How things feel today, what needs attention, and what we are doing next.</p>
      </div>
      <a class="ops-primary-link" href="/young-people-shell">Open child workspace</a>
    `;
  }
...
function normaliseItems(items = []) {
  ...
  return items.map((item) => typeof item === "string" ? { title: item, body: "", priority: "normal" } : {
...
function mergeDashboard(data = {}) {
  return { ...EMPTY_DASHBOARD, ...data, today: data.today ?? EMPTY_DASHBOARD.today, lifecycle: data.lifecycle ?? EMPTY_DASHBOARD.lifecycle, approvals: data.approvals ?? [], actions: data.actions ?? [], safeguarding: data.safeguarding ?? [], reg: data.reg ?? [], evidence: data.evidence ?? [], voice: data.voice ?? [] };
}

async function fetchJson(path, options = {}) {
  const response = await fetch(path, { credentials: "include", headers: { Accept: "application/json", ...(options.headers || {}) }, ...options });
  if (response.status === 404) return null;
  if (response.status === 401) { window.location.replace(`/login?next=${encodeURIComponent(window.location.pathname)}`); return null; }
  if (response.status === 403) { window.location.replace(`/access-denied?blocked=${encodeURIComponent(window.location.pathname)}`); return null; }
...
async function postWorkflow(path, payload = {}) {
  return fetchJson(path, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(payload) });
}
...
async function fetchDashboard() {
  ...
  return records.length ? dashboardFromRecords(records) : EMPTY_DASHBOARD;
}

async function loadDashboard() {
  ensureCareHubScreenShell();
  renderActiveScreen();
  const status = byId("opsStatus");
  if (status) status.textContent = "Loading Care Hub OS...";
  try {
    const raw = await fetchDashboard();
    renderDashboard(mergeDashboard(raw));
    if (status) status.textContent = raw === EMPTY_DASHBOARD ? "Care Hub OS ready for workflow data." : "Care Hub OS loaded.";
  } catch (error) {
    console.error("[operating-dashboard] load failed", error);
    renderDashboard(EMPTY_DASHBOARD);
    if (status) status.textContent = "Care Hub OS loaded with empty fallback data.";
  }
}
...
function renderList(id, items, emptyText = "Nothing to show.", mode = "item") {
  ...
  el.innerHTML = normalised.length ? normalised.map((item) => renderItem(item, mode)).join("") : `<p class="muted">${escapeHtml(emptyText)}</p>`;
}

function renderWorkflowControls(item, mode) {
  if (mode === "approval") return `<div class="ops-item-controls"><button type="button" class="yp-button yp-button-primary" data-workflow-action="approve" data-item-id="${escapeHtml(item.id)}">Approve</button><button type="button" class="yp-button" data-workflow-action="request_changes" data-item-id="${escapeHtml(item.id)}">Request changes</button></div>`;
  if (mode === "action") return `<div class="ops-item-controls"><button type="button" class="yp-button yp-button-primary" data-workflow-action="complete_action" data-item-id="${escapeHtml(item.id)}">Complete</button><button type="button" class="yp-button" data-workflow-action="start_action" data-item-id="${escapeHtml(item.id)}">In progress</button></div>`;
  return "";
}

function renderItem(item, mode = "item") {
  const meta = [item.owner ? `Owner: ${item.owner}` : "", item.due ? `Due: ${item.due}` : ""].filter(Boolean).join(" · ");
  const content = `<div class="ops-item-main"><strong>${escapeHtml(item.title)}</strong>${item.body ? `<p>${escapeHtml(item.body)}</p>` : ""}${meta ? `<small>${escapeHtml(meta)}</small>` : ""}</div><span class="ops-priority ops-priority-${escapeHtml(item.priority)}">${escapeHtml(item.priority)}</span>`;
  const controls = renderWorkflowControls(item, mode);
  return item.href && !controls ? `<a class="ops-item" href="${escapeHtml(item.href)}">${content}</a>` : `<div class="ops-item" data-mode="${escapeHtml(mode)}" data-item-id="${escapeHtml(item.id)}">${content}${controls}</div>`;
}

function findWorkflowItem(id) {
  for (const section of ["approvals", "actions", "safeguarding", "reg", "voice"]) {
...
async function handleWorkflowAction(action, itemId) {
  ...
  catch (error) { console.warn("[operating-dashboard] workflow action fallback", error); applyLocalWorkflowUpdate(action, itemId); if (status) status.textContent = "Updated locally. Backend workflow endpoint not yet available."; }
}
...
function applyLocalWorkflowUpdate(action, itemId) {
  ...
  if (["approve", "request_changes"].includes(action)) currentDashboard = { ...currentDashboard, approvals: removeFrom(currentDashboard.approvals), pending_approvals: Math.max(0, Number(currentDashboard.pending_approvals || 0) - 1) };
  if (action === "complete_action") currentDashboard = { ...currentDashboard, actions: removeFrom(currentDashboard.actions), safeguarding: removeFrom(currentDashboard.safeguarding), reg: removeFrom(currentDashboard.reg), voice: removeFrom(currentDashboard.voice), actions_due: Math.max(0, Number(currentDashboard.actions_due || 0) - 1) };
  if (action === "start_action") currentDashboard = { ...currentDashboard, actions: updateStatus(currentDashboard.actions, "in_progress"), safeguarding: updateStatus(currentDashboard.safeguarding, "in_progress"), reg: updateStatus(currentDashboard.reg, "in_progress"), voice: updateStatus(currentDashboard.voice, "in_progress") };
  renderDashboard(currentDashboard);
}

function bindWorkflowControls() {
  document.querySelectorAll("[data-workflow-action]").forEach((button) => button.addEventListener("click", () => handleWorkflowAction(button.dataset.workflowAction, button.dataset.itemId)));
}
