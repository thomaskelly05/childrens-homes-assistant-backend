...
function renderChildWorkspaceScreen(screen, section) {
  screen.innerHTML = `
    <section class="ops-screen-card ops-child-context-card">
      ...
      <div id="opsChildSelectorMount" class="ops-child-selector-card"><p class="muted">Loading children...</p></div>
      <div id="opsChildSectionMount" class="ops-child-section-mount"></div>
    </section>`;
  ...
  loadCareHubChildren().then((children) => {
    ...
    if (!children.length) {
      mount.innerHTML = `<div class="ops-empty-state"><h3>No children loaded yet</h3><p>Add or connect young people records to begin child-centred work.</p><a class="yp-button yp-button-primary" href="/young-people/new">Add New Child</a></div>`;
    ...
    mount.innerHTML = `
      <label class="ops-field"><span>Choose child</span><select id="opsSelectedChild">${children.map((child) => `<option value="${escapeHtml(child.id)}" ${String(child.id) === String(current?.id) ? "selected" : ""}>${escapeHtml(child.name)}</option>`).join("")}</select></label>
      <div class="ops-child-summary"><h3>${escapeHtml(current?.name || "selected child")}</h3><p>Choose an area below. It opens here inside Care Hub OS.</p></div>
      <div id="opsChildWorkspaceTabs" class="ops-menu-grid ops-child-menu-grid" data-selected-section="${escapeHtml(selectedSectionId)}">
        ${childWorkspaceMenu().map((item) => `<button type="button" class="ops-menu-card ${item.id === selectedSectionId ? "active" : ""}" data-child-section="${escapeHtml(item.id)}"><strong>${escapeHtml(item.label)}</strong><span>${escapeHtml((item.sections || []).slice(0, 4).join(" · "))}</span></button>`).join("")}
      </div>`;

    function openSection(sectionId = "about") {
      const tabs = byId("opsChildWorkspaceTabs");
      if (tabs) tabs.dataset.selectedSection = sectionId;
      document.querySelectorAll("[data-child-section]").forEach((button) => button.classList.toggle("active", button.dataset.childSection === sectionId));
      renderChildWorkspaceSection({ target: sectionMount, child: selectedChild(children) || current, sectionId });
    }
...
function normaliseItems(items = []) {
  ...
  return items.map((item) => typeof item === "string" ? { title: item, body: "", priority: "normal" } : { id: item.id || item.sourceRecordId || item.title || item.name || item.label || "", title: item.title || item.name || item.label || "Untitled item", body: item.body || item.summary || item.description || item.detail || "", priority: item.priority || item.status || "normal", owner: item.owner || item.assignee || "", due: item.due || item.due_date || item.deadline || "", href: item.href || item.url || "", status: item.status || "open", sourceRecordId: item.sourceRecordId || item.record_id || "", sourceRecordType: item.sourceRecordType || item.record_type || "" });
}
function mergeDashboard(data = {}) { return { ...EMPTY_DASHBOARD, ...data, today: data.today ?? EMPTY_DASHBOARD.today, lifecycle: data.lifecycle ?? EMPTY_DASHBOARD.lifecycle, approvals: data.approvals ?? [], actions: data.actions ?? [], safeguarding: data.safeguarding ?? [], reg: data.reg ?? [], evidence: data.evidence ?? [], voice: data.voice ?? [] }; }
